<x-slot name="title">{{ __('Sliders') }}</x-slot>
<x-slot name="header">{{ __('Sliders') }}</x-slot>
...
<div class="card" x-data="{
    ...
    <div class="card-body table-responsive">
        ...
        <x-table.table>
            <x-slot name="head">
                ...
                <x-table.heading>{{ __('Banner') }}</x-table.heading>
                <x-table.heading sortable wire:click="sortBy('title')" :direction="$sort_field === 'title'? $sort_direction : null">{{ __('Title') }}</x-table.heading>
                <x-table.heading sortable wire:click="sortBy('serial')" :direction="$sort_field === 'serial'? $sort_direction : null">{{ __('Serial') }}</x-table.heading>
            ...
            <x-slot name="body">
                @foreach($items as $item)
                    <x-table.row wire:loading.class="opacity-50" wire:target="search">
                        ...
                        <x-table.cell>
                            <img src="{{ $item->banner_src }}" style="width: 200px; height: 40px; display: block; margin: auto" />
                        </x-table.cell>
                        <x-table.cell>{{ $item->title }}</x-table.cell>
                        <x-table.cell>{{ $item->serial }}</x-table.cell>
                        <x-table.cell>
    ...
    @include('livewire.admin.slider.partials.create')
</div>
