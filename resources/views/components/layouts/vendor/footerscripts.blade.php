<!--jquery library js-->
<script src="{{asset('user/assets/js/jquery-3.6.0.min.js')}}"></script>
<!--bootstrap js-->
<script src="{{asset('user/assets/js/bootstrap.bundle.min.js')}}"></script>
<!--font-awesome js-->
<script src="{{asset('user/assets/js/Font-Awesome.js')}}"></script>
<!--select2 js-->
<script src="{{asset('user/assets/js/select2.min.js')}}"></script>
<!--slick slider js-->
<script src="{{asset('user/assets/js/slick.min.js')}}"></script>
<!--simplyCountdown js-->
<script src="{{asset('user/assets/js/simplyCountdown.js')}}"></script>
<!--product zoomer js-->
<script src="{{asset('user/assets/js/jquery.exzoom.js')}}"></script>
<!--nice-number js-->
<script src="{{asset('user/assets/js/jquery.nice-number.min.js')}}"></script>
<!--counter js-->
<script src="{{asset('user/assets/js/jquery.waypoints.min.js')}}"></script>
<script src="{{asset('user/assets/js/jquery.countup.min.js')}}"></script>
<!--add row js-->
<script src="{{asset('user/assets/js/add_row_custon.js')}}"></script>
<!--multiple-image-video js-->
<script src="{{asset('user/assets/js/multiple-image-video.js')}}"></script>
<!--sticky sidebar js-->
<script src="{{asset('user/assets/js/sticky_sidebar.js')}}"></script>
<!--price ranger js-->
<script src="{{asset('user/assets/js/ranger_jquery-ui.min.js')}}"></script>
<script src="{{asset('user/assets/js/ranger_slider.js')}}"></script>
<!--isotope js-->
<script src="{{asset('user/assets/js/isotope.pkgd.min.js')}}"></script>
<!--venobox js-->
<script src="{{asset('user/assets/js/venobox.min.js')}}"></script>
<!--classycountdown js-->
<script src="{{asset('user/assets/js/jquery.classycountdown.js')}}"></script>

<!--main/custom js-->
<script src="{{asset('user/assets/js/main.js')}}"></script>

{{-- lottie json --}}
<script src="{{asset('js/lottie-player-2.0.2.js')}}"></script>

{{-- START:: pikaday --}}
<script defer src="{{ asset('js/moment.js') }}"></script>
<script defer src="{{ asset('js/pikaday.js') }}"></script>
<link rel="stylesheet" type="text/css" href="{{ asset('css/pikaday.css') }}">
{{-- END:: pikaday --}}
